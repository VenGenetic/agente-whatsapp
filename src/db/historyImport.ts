...
async function ensureConversations(
  identities: Array<{ identifier: string; lid: string | null; name: string | null }>,
): Promise<Map<string, number>> {
  const byIdentifier = new Map<string, { lid: string | null; name: string | null }>()
...
export async function syncContactNames(contacts: Array<Partial<Contact>>): Promise<number> {
  ...
  for (const contact of contacts) {
    const identifier = contact.jid ?? contact.id
...
export async function importHistoryMessages(
  ...
  const conversationIds = await ensureConversations(
    parsedRows.map((r) => ({ identifier: r.identifier, lid: r.lid, name: r.name })),
  )

  // 3) Insertar los mensajes en lotes. `ignoreDuplicates` + el índice
  //    único sobre whatsapp_message_id evita duplicar al reimportar (el
  //    history sync puede repetirse).
  ...
  for (const batch of chunked(rows)) {
    const { error, count } = await supabase
      .from('agent_messages')
      .upsert(batch, { onConflict: 'whatsapp_message_id', ignoreDuplicates: true, count: 'exact' })
    if (error) throw error
